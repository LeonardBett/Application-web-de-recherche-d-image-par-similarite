...
const requests = {
  get: (url: string, param?: any) => instance.get(url, param).then(responseBody),
...
export const api = {
  ...
  getImage: (id: number): Promise<Blob> => instance.get(`images/${id}`, { responseType: "blob" }).then(responseBody),
